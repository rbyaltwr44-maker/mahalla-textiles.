...
function update_new() {
    let content = fs.readFileSync('src/app/(admin)/dashboard/products/new/page.tsx', 'utf-8');

    const state_old = `  const [files, setFiles] = useState<File[]>([])
  const [previews, setPreviews] = useState<string[]>([])`;
    const state_new = `  type ImageItem = { id: string; url: string; file: File };
  const [images, setImages] = useState<ImageItem[]>([])
  const [isDragOver, setIsDragOver] = useState(false)`;
    content = content.replace(state_old, state_new);

    const handle_old = `  const handleFiles = (selected: FileList | null) => {
    if (!selected) return
  ...
  const removeFile = (index: number) => {
    ...
    const handle_new = `  const handleFiles = (selected: FileList | null) => {
    if (!selected) return
  ...
  const handleDragOverSort = (e: React.DragEvent) => e.preventDefault()`;
    ...
    const upload_old = `  const uploadImages = async (): Promise<string[]> => {
    setIsUploading(true)
    const urls: string[] = []
    try {
    ...
    const upload_new = `  const uploadImages = async (): Promise<string[]> => {
    setIsUploading(true)
    const urls: string[] = []
    try {
    ...
    fs.writeFileSync('src/app/(admin)/dashboard/products/new/page.tsx', content, 'utf-8');
}

function update_edit() {
    let content = fs.readFileSync('src/app/(admin)/dashboard/products/[id]/edit/page.tsx', 'utf-8');

    const state_old = `  const [files, setFiles] = useState<File[]>([]) 
  const [previews, setPreviews] = useState<string[]>([]) 
  const [existingImages, setExistingImages] = useState<string[]>([])`;
    const state_new = `  type ImageItem = { id: string; type: 'existing' | 'new'; url: string; file?: File };
  const [images, setImages] = useState<ImageItem[]>([])
  const [isDragOver, setIsDragOver] = useState(false)`;
    content = content.replace(state_old, state_new);

    const effect_old = `        if (product.images) {
          setExistingImages(product.images)
        }`;
    const effect_new = `        if (product.images) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    fs.writeFileSync('src/app/(admin)/dashboard/products/[id]/edit/page.tsx', content, 'utf-8');
}

try { update_new(); console.log('updated new/page.tsx'); } catch(e) { console.error(e); }
try { update_edit(); console.log('updated edit/page.tsx'); } catch(e) { console.error(e); }
